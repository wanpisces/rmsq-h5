<template>
    <div class="wraper">
      <div v-for="(items,i) in list" :key="i" v-if="list.length!=0">
          <div class="list">
       ...
       <div v-if="list.length==0" class="nodata">
            <img align="center" src="https://img.sqydt.darongshutech.com/image_201805111631287995.png" width="150px" alt="">
             <div>暂无数据</div>
       </div>
       <div class="bottom-text" v-if="list.length>0">我是有底线啦~~</div>
    </div>
</template>
<script>
// vant 组件
...
export default {
  ...
  methods: {
    getData() {
      RequestGovBrokeCaseList({ group_id: localStorage.group_id||'1', type: 1 }).then(
        res => {
          console.log(res);
          if (res.code === 2000) {
